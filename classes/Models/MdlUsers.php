...
class MdlUsers
{
    ...
    public static function insertUser(User $user)
    {
        ...
        $qryInsert->bind_param('ssssiiiss', $fname, $lname, $email, $address, $city, $lat, $long, $login, $pass);
    ...
    public static function insertNewUser(User $user)
    {
        ...
        $qryInsert = $db->prepare('INSERT INTO user_s4u (
            user_login,
            user_pass,
            pass_expired)' . ' VALUES (?,?,"N")');

        // add values to the query
        $qryInsert->bind_param('ss', $user->getUserLogin() , $user->getCryptUser_pass());
    ...
    public static function updateUser(User $user)
    {
        ...
        $qryUpdate = $db->prepare(' UPDATE user_s4u SET user_pass=? ,' . '  first_name=? , last_name=? , email=? , address=?,  city_id=?, latitude=?, longitude=?, pass_expired=? ' . '             WHERE user_id=?');
        ...
        // add values to the query
        $qryUpdate->bind_param('sssssisssi', $pass, $fname, $lname, $email, $address, $city, $lat, $long, $exp, $id);
